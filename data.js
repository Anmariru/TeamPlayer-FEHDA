        ...
        favMovies: [
          {
            ...
            rating: 6.7,
            discription:
              "Nicholas Oseransky, a dentist living in a Montreal suburb, is hated by his wife Sophie and his mother-in-law, and his father-in-law has saddled him with a mountain of debt. His only friend is his receptionist Jill, who urges him to leave his wife.",
          ...
          },
          {
            ...
            rating: 8.2,
            discription:
              "Die Hard is a 1988 American action film directed by John McTiernan. It is about a police officer who single-handedly takes up the fight against gangsters who have occupied a high-rise building. It stars Bruce Willis and Alan Rickman in the leading roles. The film is the adaptation of the 1979 book Nothing Lasts Forever, written by Roderick Thorp, which is a sequel to the 1966 novel The Detective. After the great success of the film, four sequels were produced, the last of which, Die Hard - A Good Day to Die, was released in February 2013.",
          ...
          },
          {
            ...
            rating: 4,
            discription:
              "Little Miguel has a big dream. He wants to become a successful musician, just like his grandfather Ernesto de la Cruz once was. But he left his wife back then, so music doesn't exactly have a high priority in his family. One day Miguel finds his grandfather's guitar in a chapel. When he starts to play it, a door to the underworld and to music opens for him.",
          ...
          },
          {
            ...
            rating: 6.9,
            discription:
              "The plot revolves around pilot Pete Mitchell (Cruise), who is transferred with his friend to an elite training school for fighter pilots called Top Gun. There, Mitchell falls in love with the instructor Charlie (Kelly McGillis). However, when his friend dies in an accident, he falls into a serious crisis.",
            director: ["Tony Scott"],
        ...
        favMovies: [
          {
            ...
            rating: 7.6,
            description:
              "A young woman uses her seafaring talents to set sail for a fairytale island. She is accompanied on her adventure by the legendary demigod Maui.",
          ...
          },
          {
            ...
            rating: 8.2,
            description:
              "When an unconfident young woman is cursed by a wicked old witch and turned into an old woman, she sees her only hope of breaking the spell again in an immoderate but insecure young wizard who lives with his companions in a four-legged rambling castle.",
            directors: ["Hayao Miyazaki"],
          ...
          },
          {
            ...
            rating: 7.5,
            description:
              "In 19th century France, Jean Valjean is hunted for decades by the ruthless policeman Javert after committing a crime. He agrees to take care of the daughter of a factory worker. This decision changes the lives of all three characters forever.",
...
let averAge = totalAge / ourTeam.members.length
//console.log(averAge)
// Average Coding Level Calculation
let numberCoding = 0
let coding = 0
for (let index = 0; index < ourTeam.members.length; index++) {
  coding = ourTeam.members[index].level;
  numberCoding++
}
let averageCodingLevel = 0
//console.log (averageCodingLevel = coding / numberCoding)

// youngest member Calculation & oldest member
let oldestmember = ourTeam.members[2].birthday.year
let youngestMember = ourTeam.members[1].birthday.year
for (let index = 0; index <ourTeam.members.length; index++) {
  if (youngestMember <= ourTeam.members[index].birthday.year) {
    youngestMember = ourTeam.members[index].name
  }
  {
    if (oldestmember >= ourTeam.members[index].birthday.year) {
      oldestmember = ourTeam.members[index].name
    }
  } 
}
...
for (const member of ourTeam.members) {
  ...
  if (!location [city]){
    location [city] = [member.name];
  } else {
    location[city].push(member.name);
  }
}
...
for (const member of ourTeam.members){
  for (const favmovie of member.favorites.favMovies){
    for (const genre of favmovie.genres) {
      if(isNaN(commonGenres[genre])){
        commonGenres[genre] = 1
      }else{
        commonGenres[genre] = commonGenres[genre] + 1
      }
    }
  }
}
